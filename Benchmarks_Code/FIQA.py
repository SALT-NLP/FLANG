...
import json
from transformers import BertForSequenceClassification, BertTokenizerFast, ElectraTokenizerFast, ElectraForSequenceClassification, AutoModelForSequenceClassification, AutoTokenizer
import torch
...
for i, seed in enumerate(SEEDS):
    for j, lr in enumerate(LR):
        for k, bs in enumerate(BS):
            ...
            np.random.seed(seed) 
            model = AutoModelForSequenceClassification.from_pretrained(model_finetuned, num_labels=1).to(device)
