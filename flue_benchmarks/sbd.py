...
import bisect
import pandas as pd
from transformers import BertForTokenClassification, BertTokenizerFast, ElectraTokenizerFast, ElectraForTokenClassification
from torch.utils.data import DataLoader, TensorDataset 
import torch.nn.functional as F
from sklearn.metrics import f1_score

def run_sbd(gpu, modelToUse, YesorNo):
    ...
    fileNames = ["CANDRIAM_GF_LU1220230442_2015_K_P_R_A.finsbd3.json", "Credit_Suisse_Fund_I_(Lux)_2012_X_P_X_X.finsbd3.json",
    "Credit_Suisse_Fund_I_(Lux)_2012_X_P_X_X-V2.finsbd3.json","Dexia_Equities_L_2011_X_P_X_X.finsbd3.json",
    "EdR_Private_Equity_Select_Access_Fund_S.A._SICAV-SIF-Amethis_II__Sub-Fund_2018_K_X_X_X.finsbd3.json",
    "Invesco_Funds_SICAV_2013_X_P_X_A.finsbd3.json"]
    if modelToUse == 'finbert':
        ...
        tokenizer = ElectraTokenizerFast.from_pretrained('/home/ashah482/Desktop/discriminator_model', do_lower_case=False, do_basic_tokenize=True)
    else:
    ...
    addSpecialTokens = True if YesorNo == 'Yes' else False
    chunkSize = 512
    ...
    for file in predictionsFilenamesList:
        ...
        tags = torch.full((len(tokens),), label_to_id['O']) 
        ...
        for j, lr in enumerate(LR):
            for k, bs in enumerate(BS):
                ...
                if modelToUse == 'finbert':
                    ...
                    model = ElectraForTokenClassification.from_pretrained('/home/ashah482/Desktop/discriminator_model', num_labels=num_labels).to(device)
                else:
    ...
    df.to_csv(f'GridSearchResults/SBD_{addSpecialTokens}_{modelToUse}_small_BS.csv', index=False)